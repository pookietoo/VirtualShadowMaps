...
namespace vsm::internal
{
	...
	inline constexpr char kBuildTag[] =
	    "0.9.86 CODE-REVIEW FIXES + REFACTOR on top of 0.9.85 (features P4+A1+A5). UNTESTED — user will test when able; this is intended to be behavior-preserving vs 0.9.85. DEDUP: ForEachLightFace (the light x 6-face viewport/faceVP/frustum-plane math, now shared by RenderCasterPass, BakeOneLightStatic and RenderTranslucentPass), EmitCasterDraw (shared per-caster map+bind+draw tail), PoseFreezeLight (shared P2/P4 pose restore). FIXES: bakedPose/dirtyLights no longer leak (pruned to currently-collected lights each frame); visibleCasters no longer counts glass draws; init double-bake removed (CreateStaticCacheResources keeps lastStaticCasterCount so it doesn't re-invalidate a second time); static_assert(kReverseZ) guards the P4 fullscreen-triangle block-clear; named kCubeFacesWide/kCubeFacesTall replace bare 3/2 face literals. Compiles /W4 0/0. Deferred: StaticShadowCache class extraction (to pair with perf opts O5/O8 — see notes/VSM_OPTIMIZATION_RESEARCH.md). Features unchanged from 0.9.85: A1 far-distance fade [shader, always on]; P4 incremental cache [config.incrementalCache, default OFF, needs cacheStaticShadows]; A5 colored translucent shadows [config.translucentShadows, default OFF, t112 transmittance atlas]. A5 REQUIRES the matching CommunityShaders.dll + shaders (t112). TEST: baseline OFF unchanged vs 0.9.84; A1 softens the shadow near a light's radius; toggle P4 (with cache) and A5 (glass dims/tints) independently.";
