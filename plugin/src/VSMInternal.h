...
namespace vsm::internal
{
	...
	inline constexpr char kBuildTag[] =
	    "0.9.84 FLICKER FIX (baseline, independent of P2/P3): shadows in a many-light room blinked on/off — even with P2+P3 OFF and even while PAUSED in the console. ROOT: the engine ROTATES which lights are IsShadowLight()/activeShadowLights every RENDER frame (its own shadow-light management, runs even when paused), so our collected set churned 5<->8 frame-to-frame and each dropped light's shadow vanished that frame. FIX = collection DEBOUNCE: keep a light shadowed for a GRACE window (kLightGraceFrames=30) after it last appeared — refresh a per-light cache for lights collected this frame, then RE-ADD any recently-seen light missing this frame (using its cached record). Stabilizes the set to the UNION of recently-active shadow lights = exactly the mod's goal (shadow the lights the engine drops). C++-only, NO shader change (still needs the 0.9.81 PCF+per-light shader -> recompile CS if coming from <=0.9.80). Prior fixes kept: 0.9.83 P2 pose-freeze (fire shadow steady), P1/P2/P3 perf modules (P2/P3 default-OFF toggles). TEST: the light-room flicker should be gone with all toggles OFF; then re-validate P2/P3. P4 still deferred.";
